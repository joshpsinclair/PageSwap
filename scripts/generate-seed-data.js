...
const LOCALES = [
    'en', 'es', 'fr', 'de', 'it', 'pt_BR', 'ru', 'ja', 'ko', 'zh_CN',
    'ar', 'pl', 'tr', 'nl', 'sv', 'no', 'da', 'fi', 'el', 'he', 'th', 'vi'
];
...
function generateProfileImageUrl() {
    ...
    // 70% valid avatar ID
    if (rand < 0.70) {
        return faker.helpers.arrayElement(AVATAR_IDS);
    }
...
console.log(`   - ${users.filter(u => u.profileImageUrl && !AVATAR_IDS.includes(u.profileImageUrl)).length} have invalid avatar`);
